  ...
  pane.innerHTML = `
    <div id="carrito-${tabId}">
      <div class="cart-header cart-header-venta">
        ...
        <span class="ch-title">Venta ${tabId.replace('venta','')}</span>
...
async function dropTab(tabId,Confirmation = 0) {
  ...
  try {
    ...
    if (!response.success) {
      // Si la venta YA no existe en la base, el objetivo igual está cumplido:
      // hay que quitar la pestaña. Antes se mostraba un error y la pestaña
      // quedaba "zombie" — visible, con productos, imposible de cerrar ni de
      // cobrar — bloqueando la caja hasta recargar.
      const mensaje = String(response.error || "").toLowerCase()
      const yaNoExiste = mensaje.includes("no encontrada") || mensaje.includes("no existe")

      if (yaNoExiste) {
        console.warn("[TABS] La venta ya no existía en BD; se cierra la pestaña igualmente:", idVenta)
        performTabRemoval(tab, containerTab, pane, tabId)
        return
      }
    ...
    // Última red de seguridad: ofrecer cerrar la pestaña de todos modos para
    // que la caja nunca quede bloqueada por un problema de conexión.
    const decision = await Swal.fire({
      icon: 'error',
      title: 'No se pudo eliminar la venta',
      text: error.message + ' ¿Quieres cerrar la pestaña de todas formas?',
      showCancelButton: true,
      confirmButtonText: 'Sí, cerrar pestaña',
      cancelButtonText: 'Cancelar',
      confirmButtonColor: '#dc3545'
    })
