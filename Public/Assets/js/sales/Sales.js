...
// Marca visualmente el carrito según sea de MESA o de VENTA normal, para que
// el cajero identifique de inmediato dónde está (colores distintos + banner).
function marcarModoCarrito(cartId) {
  const col = document.querySelector(".pos-col-cart")
  if (!col) return
  const esMesa = isTableTab(cartId)
  col.classList.toggle("mode-mesa", esMesa)
  col.classList.toggle("mode-venta", !esMesa)
}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // ---------------------------------------------------------------------------
  // SINCRONIZACIÓN DEL CARRITO ACTIVO CON LA PESTAÑA VISIBLE
  // ---------------------------------------------------------------------------
  // CLAVE: la pestaña fija "Venta 1" es HTML estático y NO tenía listener, así
  // que al volver a ella desde una mesa, `currentCartId` quedaba apuntando a la
  // mesa. Resultado: los productos se veían en pantalla (addToCart usa el DOM)
  // pero transferir/cobrar leía el carrito equivocado (vacío).
  // Este listener de Bootstrap se dispara al mostrarse CUALQUIER pestaña y deja
  // `currentCartId` siempre alineado con lo que se ve.
  const ventasTabs = getById("ventasTabs")
  if (ventasTabs) {
    ventasTabs.addEventListener("shown.bs.tab", (e) => {
      const href = e.target.getAttribute("href")
      const cartId = href ? href.substring(1) : null
      if (cartId) {
        setActiveCart(cartId)
        marcarModoCarrito(cartId)
      }

      // Al SALIR de una mesa sin productos, liberarla (mesa vacía = no ocupada).
      const prevHref = e.relatedTarget && e.relatedTarget.getAttribute("href")
      const prevCartId = prevHref ? prevHref.substring(1) : null
      if (prevCartId && prevCartId !== cartId && typeof liberarMesaSiVacia === "function") {
        liberarMesaSiVacia(prevCartId)
      }
    })
  }

  // Estado inicial (Venta 1)
  marcarModoCarrito(currentCartId)
...
function openDailyReportModal() {
  ...
  fetch('?pg=sales&action=dailyBills&ajax=1', {
    method: 'GET',
    headers: {
