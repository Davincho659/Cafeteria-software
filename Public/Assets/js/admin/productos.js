...
/**
 * Campo de código de barras del formulario de producto.
 * - El botón "Escanear" enfoca el campo (para que el lector escriba ahí) y da
 *   una pista visual. En pantalla táctil, tocar el campo abre el teclado.
 * - El lector termina el código con Enter; se evita que eso envíe el formulario.
 */
function setupBarcodeField() {
    const input = document.getElementById('prod_codigoBarras');
    ...
    if (btn) {
        btn.addEventListener('click', function () {
            ...
            input.select();
            if (hint) {
                hint.textContent = '📷 Esperando escaneo... pasa el producto por el lector.';
                hint.classList.add('text-primary');
            }
        });
    }

    // El Enter del lector NO debe enviar el formulario; solo confirma el código.
    input.addEventListener('keydown', function (e) {
        if (e.key === 'Enter') {
            e.preventDefault();
            input.blur();
            if (hint) {
                hint.textContent = '✅ Código capturado: ' + input.value;
...
function openCategoryModal() {
    ...
    if (window.imgPickers && window.imgPickers.cat) window.imgPickers.cat._reset();
...
function openProductModal() {
    ...
    if (window.imgPickers && window.imgPickers.prod) window.imgPickers.prod._reset();
    document.getElementById('prod_codigoBarras').value = '';
...
function openEditCategory(id) {
    fetch(`index.php?pg=product&action=getCategory&id=${id}`)
        ...
        .then(data => {
            ...
            if (window.imgPickers && window.imgPickers.cat) {
                window.imgPickers.cat._reset();
                if (c.imagen && c.imagen !== 'default.png') {
                    window.imgPickers.cat._showCurrent('assets/img/categories/' + c.imagen);
                }
            }
...
function openEditProduct(id) {
    fetch(`index.php?pg=product&action=getProduct&id=${id}`)
        ...
        .then(data => {
            ...
            document.getElementById('prod_codigoBarras').value = p.codigoBarras || '';
            if (window.imgPickers && window.imgPickers.prod) {
                window.imgPickers.prod._reset();
                if (p.imagen && p.imagen !== 'default.png') {
                    window.imgPickers.prod._showCurrent('assets/img/products/' + p.imagen);
                }
            }
