...
async function loadPurchasesHistory() {
    // El historial es solo para administradores. Los empleados pueden registrar
    // compras pero no consultar el historial (el servidor también lo bloquea).
    if (window.APP_IS_ADMIN === false) return;
    if (!document.getElementById('filterSupplier')) return;
