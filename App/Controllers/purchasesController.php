...
class PurchasesController {
    ...
    public function createDetailedPurchase() {
        ...
        try {
            ...
            $idUsuario = $_SESSION['usuario_id'] ?? null; // siempre desde la sesión

            // Calcular total en servidor para evitar desajustes. Cada línea se
            // valida (numérica, cantidad > 0, precio >= 0 y dentro de topes).
            $total = 0;
            foreach ($data['productos'] as $item) {
                $cantidad = Validator::cantidad($item['cantidad'] ?? null, false, 'La cantidad de compra');
                // En compras el precio puede ser 0 (ej. insumo de regalo)
                $precio = Validator::precio($item['precioUnitario'] ?? 0, 0, 'El precio de compra');
    ...
    public function createQuickPurchase() {
        ...
        try {
            ...
            $total = Validator::monto($data['total'], 'El total de la compra');
