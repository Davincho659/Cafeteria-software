...
class Expenses {
    ...
    /**
     * Totales del conjunto filtrado de gastos (mismos filtros que getFiltered).
     * Para la fila de totales del reporte, en una sola consulta.
     */
    public function getFilteredTotals($filtros = []) {
        $sql = "SELECT
                    COALESCE(SUM(g.monto), 0) AS totalGastos,
                    COUNT(*) AS cantidadGastos,
                    COALESCE(SUM(CASE WHEN g.tipo='producto' THEN g.monto ELSE 0 END), 0) AS totalProducto,
                    COALESCE(SUM(CASE WHEN g.tipo='externo'  THEN g.monto ELSE 0 END), 0) AS totalExterno
                FROM gastos g
                WHERE 1=1";
        ...
        return [
            'totalGastos'    => (float)($row['totalGastos'] ?? 0),
            'cantidadGastos' => (int)($row['cantidadGastos'] ?? 0),
            'totalProducto'  => (float)($row['totalProducto'] ?? 0),
            'totalExterno'   => (float)($row['totalExterno'] ?? 0),
        ];
    }
