...
class Purchases {
    ...
    /**
     * Totales del conjunto filtrado de compras (mismos filtros que getFiltered).
     * Para la fila de totales del reporte, en una sola consulta.
     */
    public function getFilteredTotals($filtros = []) {
        $sql = "SELECT COALESCE(SUM(c.total), 0) AS totalComprado, COUNT(*) AS cantidadCompras
                FROM compras c
                LEFT JOIN proveedores p ON c.idProveedor = p.idProveedor
                WHERE 1=1";
        ...
        return [
            'totalComprado'    => (float)($row['totalComprado'] ?? 0),
            'cantidadCompras'  => (int)($row['cantidadCompras'] ?? 0),
        ];
    }
